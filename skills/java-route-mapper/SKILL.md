...
#### 1.2.1 Spring MVC WebMvcConfigurer 路径前缀

`WebMvcConfigurer#configurePathMatch` 中的 `PathMatchConfigurer#addPathPrefix` 会改变 Controller 的最终对外 URL，必须在枚举 `@RequestMapping` 路由前先提取并应用。

**强制执行步骤：**

1. 扫描所有 `WebMvcConfigurer` 实现类和 `@Configuration` 配置类
2. 搜索 `configurePathMatch`、`PathMatchConfigurer`、`addPathPrefix`
3. 提取每条 `addPathPrefix(prefix, predicate)` 的前缀、匹配条件、源码位置
4. 将匹配条件应用到所有 `@Controller` / `@RestController` 类
5. 输出完整路径时按 `context-path + addPathPrefix + 类级别路径 + 方法级别路径` 组合
6. 在路由详情中标注前缀来源；无法静态判定的 predicate 必须记录为“需人工确认”，禁止静默忽略
...
若 `@AdminController` 类上存在 `@RequestMapping("/users")`，方法上存在 `@GetMapping("/{id}")`，则最终 URL 必须输出为 `/admin/users/{id}`，不能只输出 `/users/{id}`。
...
- `WebMvcConfigurer#configurePathMatch` / `PathMatchConfigurer#addPathPrefix` 路径前缀配置
...
5. **Spring WebMvcConfigurer** - `addPathPrefix` 动态添加的 Controller 前缀
6. **Web Service 路径** - 如 `/services/`, `/ws/`
...
| 路由路径不完整 | 检查类级别的 `@RequestMapping`、上下文路径配置、`WebMvcConfigurer#configurePathMatch` / `addPathPrefix` |
