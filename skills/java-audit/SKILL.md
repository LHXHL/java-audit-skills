...
4. 读取 `references/workflow.md`；若是漏洞审计，再读取 `references/evidence-standard.md`、`references/vulnerability-hypotheses.md` 和 `references/evidence-matrix.md`。
...
6. 根据鉴权面和代码证据创建 `workspace/evidence/vulnerability-type-screening.md`，将常见 Java 漏洞族逐项列成初筛表并完成状态标记。
7. `[x]` 和 `[?]` 漏洞族都必须生成至少一个 `VULN-CAND-xxx` 候选并进入深度审计；同一漏洞族下发现多个独立入口、根因或传播链时，必须拆成多个候选。
8. 为每个具体候选创建并维护独立证据矩阵，逐项证明入口、参数来源、鉴权方式、sink、传播链、触发条件、payload、BurpSuite 请求包和影响；候选最终状态必须闭环为 `确认`、`降级` 或 `放弃`，不能停留在 `候选`。
9. 候选满足确认门槛后，排查同源路由；只有逐条严格确认的其他入口才能并入确认漏洞。
10. 为主确认入口构造完整 payload 与原始 HTTP 请求包；其他确认入口可写最小差异请求摘要。
11. 生成最终报告前，必须运行 `scripts/validate_evidence_closure.py <workspace>`；任一 `[x]`/`[?]` 没有候选、没有证据矩阵或没有闭环状态，或初筛表仍有 `[ ]` 时，必须返工，不能出报告。
12. 生成 Markdown 报告并运行 `scripts/validate_report.py`；校验失败必须返工报告。
...
- 不得把 `workspace/evidence/` 中的漏洞族初筛表、证据矩阵或被阻断/不可达/证据不足的同源路由复制进最终报告。
- 不得在存在未闭环 `[x]`/`[?]` 漏洞族时生成最终报告；不能只在初筛表标记而不生成候选、不深审或不写降级/放弃原因。
