...
4. 读取 `references/workflow.md`；若是漏洞审计，再读取 `references/evidence-standard.md` 和 `references/vulnerability-hypotheses.md`。
...
8. 使用漏洞假设库生成内部候选并逐项找成立证据；无法证明时放弃或降级，不把“漏洞不存在/已排除漏洞类型”写进报告。
