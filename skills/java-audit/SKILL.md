...
7. 每个 `[x]` 漏洞族必须生成至少一个 `VULN-CAND-xxx` 候选并进入深度审计；同一漏洞族下发现多个独立入口、根因或传播链时，必须拆成多个候选。
...
11. 生成最终报告前，必须运行 `scripts/validate_evidence_closure.py <workspace>`；任一 `[x]` 没有候选、没有证据矩阵或没有闭环状态时，必须返工深度审计，不能出报告。
...
- 不得在存在未闭环 `[x]` 漏洞族时生成最终报告；不能只在初筛表勾选而不生成候选、不深审或不写降级/放弃原因。
