...
8. 使用漏洞假设库生成内部候选；对高价值候选在 `workspace/evidence/` 同步维护证据矩阵，不把矩阵复制进最终报告。
9. 候选满足确认门槛后，排查同源路由；只有逐条严格确认的其他入口才能并入确认漏洞。
10. 为主确认入口构造完整 payload 与原始 HTTP 请求包；其他确认入口可写最小差异请求摘要。
11. 生成 Markdown 报告并运行校验脚本；校验失败必须返工报告。
...
- 不得把 `workspace/evidence/` 中的证据矩阵或被阻断/不可达/证据不足的同源路由复制进最终报告。
...
- `references/evidence-matrix.md`：漏洞审计中维护候选证据矩阵、同源路由排查和根因归并时读取。
