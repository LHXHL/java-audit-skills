...
- 当前环境没有可创建/调度 Claude team agent 的能力，且用户要求完整流水线：不要退化成单进程全跑；写 `pipeline_blocked.md`。

## 成功标准

- 开始前确认是否具备真实 agent/team 调度能力；不能确认时阻塞，不模拟。
- 能力确认必须覆盖当前 `claude -p` 非交互回合的完整生命周期：创建 agent、等待 worker 完成、启动独立 QA、处理返工、关闭 team、让 CLI 自然返回；只具备 agent 创建能力不算通过。
- 工具名、模型自称、计划表、可见命令名或推断不能作为完整生命周期证据；没有用户请求或平台上下文提供的外部已验证证据时，当前 `claude -p` 回合必须把完整生命周期能力记为“不可确认”并阻塞。
- 普通用户请求“启动流水线/完整审计/使用本 skill 审计源码”不等于提供完整 team 生命周期证据；不要为了测试能力而创建 worker。
...
5. 做能力确认：
   - 只读取用户请求和平台上下文中已经存在的外部生命周期证明；不得通过启动业务 worker 来“试验”能力。
   - 若外部证明同时覆盖创建、等待、QA、返工/阻塞处理、关闭和当前 CLI 自然返回：写入 `team_execution.md` 初始记录，启动 team 流水线。
   - 若没有外部证明、只有工具名、只有模型自称、只有 agent 创建能力或只有平台能力推断：写 `team_execution.md` 与 `pipeline_blocked.md`，说明完整 team 生命周期不可确认、未运行阶段和继续条件；停止。
...
- `claude -p` 非交互模式可能只能创建 agent，却不能保证完整等待、关闭和自然返回；这种情况应早阻塞，不应先跑业务 worker。
...
- 用户请求未提供外部已验证的完整 Claude team 生命周期证据：初始化后生成 `team_execution.md` 和 `pipeline_blocked.md`，不启动顺序模拟或长任务 worker。
- 只有工具名、能力名称、模型自称或“看起来可以创建 agent”的证据：仍按不可确认处理并阻塞。
- team 执行记录缺失、QA 报告缺失、阶段门禁不通过：停在当前阶段返工。
- 若无法在当前 CLI 会话继续推进或等待会造成挂起：写 `pipeline_blocked.md` 后立即返回，不持续等待。
- 普通 `claude -p` 单次提示无法在运行开始前证明本次会自然返回；除非平台明确提供完整生命周期证据，否则初始化后阻塞。
...
| 正例 | “用 Claude team 跑 route/auth/tracer/SQL/反序列化，并让质检员检查；平台已提供可等待、QA、关闭并自然返回的 team 生命周期证明。” | 触发，必须生成 `team_execution.md` 和 agent 命名 QA |
| 反例 | “扫描 pom.xml 有多少 CVE 命中。” | 不触发，切换 `java-vuln-scanner` |
| 反例 | “追踪 /admin/login 到 DAO 的调用链。” | 不触发，切换 `java-route-tracer` |
| 边界例 | “完整流水线，但请求中没有完整 team 生命周期证明。” | 初始化后写 `team_execution.md` 和 `pipeline_blocked.md`，不顺序模拟 |
