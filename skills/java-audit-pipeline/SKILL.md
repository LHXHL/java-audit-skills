...
- `scripts/route_extractors/`、`scripts/trace_helpers/`：项目专用路由枚举脚本和调用链追踪 helper；脚本输出可作为结构化证据来源。
- `structured/`：机器可校验中间产物，包括 route mechanisms、routes、dispatchers、sink candidates、trace tasks、trace sinks、candidate packages 和 coverage report。
...
- `vuln_report/` 必须使用 `java-vuln-scanner` 六段模板，并通过仓库级维护脚本 `tools/skill-maintenance/validators/validate_vuln_output.py`。
...
- 路由、sink、任务分派和专项交接必须优先使用 `structured/` 中的 JSON/JSONL；Markdown 报告只作为人类可读交付物。
- 不做全量路由 LLM 深审；必须先用脚本化索引和风险评分筛选高价值候选。
...
4. 读取 `references/STRUCTURED_EVIDENCE_FLOW.md`，确认 `structured/` 文件、schema、风险评分和跨专项派发规则。
5. 读取 `references/quality_check_templates.md`，确认 QA 文件、最终质量报告和阻塞报告模板。
6. 做能力确认：
...
7. 阶段1：
   - `agent-1-recon` 读取 `agent_1_recon_instructions.md` 做路由机制侦查、模块切分和 extractor 任务规划；共享 extractor 写入 `scripts/route_extractors/`；通过 `agent-7-x` 校验后启动 `agent-1-N`。
   - `agent-1-N` 使用 `java-route-mapper` 并行提取模块路由，必要时只在 `scripts/route_extractors/agent-1-{N}/` 写 worker 专属 extractor 修正，写入 Markdown 与 `structured/routes.jsonl`、`structured/dispatchers.jsonl`；每个完成后由 `agent-7-x` 校验，失败则返工。
...
8. 阶段2：
   - `agent-4a` 读取 `agent_4a_instructions.md`，结合 routes、dispatchers、sink candidates 和鉴权上下文生成风险评分与 `candidate_packages.jsonl`。
   - `agent-4b` 读取 `agent_4b_instructions.md` 聚合组件版本证据、鉴权发现和 sink-first 候选。
   - 两者都由 `agent-7-x` 校验通过后才能进入阶段3。
9. 阶段3：
   - `agent-5-route-tracer` 读取 `agent_5_instructions.md`，从 `candidate_packages.jsonl` / `trace_tasks.jsonl` 生成高价值追踪批次。
   - `agent-5-N` 使用 `java-route-tracer` 并行追踪；批量、高风险候选和重复调用链模式必须先写 `scripts/trace_helpers/agent-5-{N}/` 项目专用 helper，再写 Markdown 与 `structured/trace_sinks.jsonl`；每个批次完成后由 `agent-7-x` 校验。
10. 阶段4：
   - 负责人根据 `trace_sinks.jsonl` 和 `sink_candidates.jsonl` 的 `dispatch_to` 启动对应 `agent-6x` 专项 worker。
...
11. 阶段5：
   ...
   - 负责人可运行本地 validator 作为最终兜底，尤其是结构化 coverage 与跨专项一致性检查；validator 不能替代 `agent-7-x` 质检员。

## References 读取时机

- `references/TEAM_ORCHESTRATION.md`：启动流水线前必须读取。
- `references/PIPELINE_INITIALIZATION.md`：解析输入后、创建任何 agent 前必须读取。
- `references/STRUCTURED_EVIDENCE_FLOW.md`：创建结构化中间产物、风险评分或专项派发前读取。
...
- `structured/` 中的跨专项 sink 必须按 `sink_types` 与 `dispatch_to` 派发；多标签 sink 必须交给所有对应专项，结构类 SQL sink 交给 SQL 专项。
- 不能因为路由总量大而让 LLM 全量深审；必须先结构化索引、评分、再深审高价值候选。
