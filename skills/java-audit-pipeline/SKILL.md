...
阶段4: 漏洞深度检测（按需并行）
  ...
  └─ agent-6e-deserialization-auditor: /java-deserialization-audit → 反序列化漏洞检测（含可利用前置条件） → agent-7-x 校验 → 通过后关闭
        ↓
阶段5: 汇总报告
...
task-17e: agent-6e-deserialization-auditor 反序列化漏洞检测 (blockedBy: [13], 按需启动)
...
10. **阶段4 调度**：负责人读取调用链报告，按 sink 类型双池调度启动 agent-6x（worker 池上限 `{max_concurrent_agents}`，质检员池上限 `ceil({max_concurrent_agents} / 2)`）；worker 完成立即替补不等校验；无对应 sink 的 agent-6x 跳过，直接标记 completed。`agent-6e-deserialization-auditor` 的启动条件为：调用链报告中存在 `DESERIALIZE` sink，或 `vuln_report` / `cross_analysis/component_vulnerabilities.md` 命中 Fastjson、XStream、Shiro RememberMe、Log4j/JNDI、JDBC、CommonsCollections 等反序列化相关组件。
11. **质检员池调度策略**：
...
### Agent-6e-deserialization-auditor: 反序列化审计员

```
角色: agent-6e-deserialization-auditor (反序列化审计员)
等待: 所有 agent-5-N 调用链追踪完成，且调用链中存在 DESERIALIZE sink，或组件漏洞报告命中反序列化相关组件
...
脚本目录: {output_path}/scripts/（所有运行时生成的临时脚本、中间文件必须写入此目录，禁止写入 /tmp 或其他临时目录）
输入:
  - {output_path}/route_tracer/ 下含 DESERIALIZE sink 的调用链报告（含鉴权风险信息）
  - {output_path}/vuln_report/ 下的组件漏洞报告
  - {output_path}/cross_analysis/component_vulnerabilities.md（如存在）
任务: 基于调用链和组件证据做精准反序列化漏洞检测，确认入口可达性、用户可控性、组件/JDK/gadget 条件和 payload 可测试性；组件版本命中只能作为输入证据，不能直接判定漏洞成立
```

**关键要求：可利用前置条件**（同 agent-6a），并额外包含：

- **组件/JDK 条件**：明确版本是否满足
- **gadget 条件**：明确 classpath 中是否存在可用 gadget
- **网络条件**：明确是否需要 DNS/LDAP/RMI 出网
- **Payload 约束**：只输出授权测试用途的低破坏验证 payload
...
| 反序列化 | `DESERIALIZE`, `ObjectInputStream`, `readObject`, `readUnshared`, `readExternal`, `XMLDecoder`, `JSON.parseObject`, `JSON.parse`, `new XStream`, `fromXML`, `DriverManager.getConnection`, `DataSource.getConnection`, `rememberMe`, `CookieRememberMeManager`, `JndiLookup`, `\$\{jndi:`, `commons-collections`, `InvokerTransformer`, `TiedMapEntry` | agent-6e-deserialization-auditor |

**判断逻辑：**
1. 负责人读取 `{output_path}/route_tracer/` 下所有调用链报告
2. 在报告中搜索上述特征关键词（支持正则匹配）
3. 对 agent-6e，还需读取 `{output_path}/vuln_report/` 与 `{output_path}/cross_analysis/component_vulnerabilities.md`，若命中反序列化相关组件，即使调用链未出现 `DESERIALIZE` sink，也启动 agent-6e 做触发点确认
4. 仅启动有对应 sink 或组件证据的 agent，无对应证据则跳过该 agent，直接标记任务为 completed
5. 优先方案：直接读取 java-route-tracer 输出报告中的 **Sink 识别章节**，该章节已完成完整的 Sink 分类
...
├── deserialization_audit/     # 阶段4 - agent-6e-deserialization-auditor（含可利用前置条件）
├── decompiled/                # 反编译输出
...
| agent-6e-deserialization-auditor 输出 | `/java-deserialization-audit` |
