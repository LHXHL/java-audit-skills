...
| 6 | 多入口聚合校验 | 每个漏洞详情含“受影响入口”表；同根因多入口按 `java-shared/VULNERABILITY_GROUPING.md` 合并为一个漏洞编号；不同鉴权/组件/sink/payload/修复条件已拆分 | {填写：入口表覆盖、合并/拆分是否合理} | {✅/❌} |
| 7 | 反序列化专项校验（仅 agent-6e） | 组件版本命中未被直接判漏洞；每个漏洞含组件/JDK/gadget/网络条件、Burp 请求和 payload；安全 sink 已摘要说明 | {填写：不适用或具体检查结果} | {✅/❌/-} |
...
- {✅/❌→✅} 多入口聚合: {填写：同根因合并/不同条件拆分/受影响入口表检查结果}
