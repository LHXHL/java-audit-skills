...
- 只能确认 agent 创建、不能确认等待/QA/关闭/自然返回时，阻塞在初始化后，不启动业务 worker。
- 不得把工具名或模型自称写成完整生命周期证据；缺少外部已验证证据时写“不可确认”。
- 用户请求没有提供外部完整生命周期证明时，阻塞点固定为“初始化后、业务 worker 启动前”；不得生成 recon、组件扫描、交叉分析、调用链或专项漏洞产物。
- 阻塞报告和 `team_execution.md` 中不要写具体工具名作为可用证据或反面说明；统一写“未提供外部已验证生命周期证明”。
...
若阻塞发生在普通 `claude -p` 初始化后、业务 worker 启动前，第 1 节只记录“初始化”，阶段1到阶段5均写入第 3 节“未运行阶段”。
