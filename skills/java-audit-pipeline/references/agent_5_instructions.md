...
`agent-5` 把阶段2的待追踪入口拆成明确批次，供 `java-route-tracer` worker 逐批处理。它不追踪代码、不判断漏洞、不修改鉴权结论。
...
4. 每批建议 `1-10` 个入口；复杂入口、WebService、网关方法或参数分支多的入口单独成批。
...
| 序号 | 优先级 | 入口 | 方法 | 参数摘要 | 上游鉴权标签 | 证据文件 |
|---:|---|---|---|---|---|---|
| {number} | P0/P1/C1/P2 | {route/service/method} | {HTTP method 或 Java method} | {params} | {无鉴权/绕过发现编号/有鉴权/未提供} | {path/section} |
