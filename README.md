...
6. 漏洞审计必须将常见 Java 漏洞族列成内部初筛表逐项检查；每个 `[x]` 或 `[?]` 漏洞族必须生成 `VULN-CAND-xxx` 候选、进入深度审计并形成闭环状态。
7. AI 生成最终报告前先运行 evidence 闭环校验；任一 `[x]`/`[?]` 没有候选、没有证据矩阵或状态仍为“候选”时，必须返工；初筛表不得保留 `[ ]`。
...
`validate_evidence_closure.py` 会检查漏洞族初筛表中每个 `[x]`/`[?]` 是否生成候选、是否有证据矩阵、是否闭环为确认/降级/放弃，并禁止最终初筛表保留 `[ ]`。它只校验流程闭环，不判断漏洞真假。
